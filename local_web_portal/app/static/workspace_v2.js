(function () {
  ...
  const buildPlanningCards = () => {
    ...
    (Array.isArray(data.plot_points) ? data.plot_points : []).slice(0, 4).forEach((x, i) => {
      beatCards.push({
        ...
        kind: "plot_point",
        tone: "cool",
        title: x.position || "Plot Beat",
        meta: x.timestamp || "",
        previewText: preview(x.content, 180),
        content: String(x.content || "").trim(),
        sourceLabel: "Plot beat",
        editable: false,
        asset: x,
      });
    });
  ...
  function renderManuscriptEmptyState() {
    ...
    const outlineUrl = `/console/manuscript/outline${sfx}`;
    const planningUrl = `/console/manuscript/planning${sfx}`;
    let hint = '';
    if (isOutline) {
  ...
  function renderManuscript() {
    ...
    if (r.draft && !selectedPlan) {
      ...
      if (refs.download) refs.download.href = downloadJobId && r.chapter ? `/jobs/${downloadJobId}/download/chapter/${r.chapter}` : "#";
      ...
      if (refs.planSource) refs.planSource.textContent = selectedPlan ? (selectedPlan.editable ? "Editable" : "Inspect") : (r.outline ? txt("chapterOutline", "Chapter Outline") : txt("chapterBrief", "Chapter Brief"));
  ...
  async function refreshGroups() {
    if (!runId) return;
    try { const r = await fetch(`/api/runs/${runId}/memory-banks`, { cache: "no-store" }); if (!r.ok) throw new Error(); const j = await r.json(); groups = Array.isArray(j.groups) ? j.groups : []; renderMemory(); setStatus(txt("refreshed", "Memory refreshed."), false); } catch (_) { setStatus(txt("refreshFailed", "Refresh failed. Try again shortly."), true); }
  }
  ...
  async function saveDraft() {
    ...
    try {
      const resp = await fetch(`/api/runs/${runId}/chapters/${draft.chapter}/content`, {
        method: "POST",
  ...
  async function saveOutline() {
    ...
    setManuscriptStatus(txt("saving", "Saving..."), false);
    try {
      const resp = await fetch(`/api/runs/${runId}/outlines/${outline.id}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function saveBrief() {
    ...
    setManuscriptStatus(txt("saving", "Saving..."), false);
    try {
      const resp = await fetch(`/api/runs/${runId}/memory-banks/chapter_briefs/entries/${brief.id}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
