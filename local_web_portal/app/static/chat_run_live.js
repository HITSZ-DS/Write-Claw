(function () {
  ...
  const TOOL_META = {
    inspect_workspace: { icon: "WS", label: "inspect_workspace" },
    plot_strategy: { icon: "PL", label: "plot_strategy" },
    retrieve_context: { icon: "CTX", label: "retrieve_context" },
    enrich_character: { icon: "CHR", label: "enrich_character" },
    enrich_world: { icon: "WRLD", label: "enrich_world" },
    draft_chapter: { icon: "DR", label: "draft_chapter" },
    rewrite_chapter: { icon: "RW", label: "rewrite_chapter" },
    sync_storyboard: { icon: "SB", label: "sync_storyboard" },
    sync_characters: { icon: "CH", label: "sync_characters" },
    sync_world: { icon: "WD", label: "sync_world" },
    ask_user: { icon: "ASK", label: "ask_user" },
    finalize: { icon: "OK", label: "finalize" },
  };

  const CHECKPOINT_CHIPS = ["Continue as-is", "Raise tension next chapter", "Slow down for detail", "Focus more on character", "Add more world texture"];
  const CHECKPOINT_MEMORY_BANKS = [
    { slug: "chapter_briefs", label: "Chapter briefs" },
    { slug: "scene_cards", label: "Scene cards" },
    { slug: "entity_state", label: "Entity state" },
    { slug: "relationship_state", label: "Relationship state" },
    { slug: "world_state", label: "World state" },
    { slug: "continuity_facts", label: "Continuity facts" },
    { slug: "revision_notes", label: "Revision notes" },
    { slug: "working_set", label: "Working set" },
  ];

  function toolMeta(name) {
    return TOOL_META[name] || { icon: "TL", label: name || "tool" };
  }
  ...
  function buildDisplayItems(entries) {
    ...
    for (const entry of entries) {
      ...
      if (name === "claw_loop_start" || name === "claw_native_loop") {
        items.push({ type: "banner", id: `loop-${entry.stamp}`, title: "Claw loop started", body: `max_steps=${kv.max_steps || "-"}${kv.enabled_actions ? ` | tools=${kv.enabled_actions}` : ""}` });
        continue;
      }
      ...
      if (name === "claw_write_result" || name === "claw_candidate_update") {
        for (let i = items.length - 1; i >= 0; i -= 1) {
          if (items[i].type === "tool-card" && items[i].status === "running") {
            ...
            items[i].result = `length=${kv.length || "-"} chars${kv.issues && kv.issues !== "0" ? ` | issues=${kv.issues}` : ""}`;
            break;
          }
        }
        continue;
      }
      if (name === "claw_quality_ok") { items.push({ type: "banner", id: `quality-${entry.stamp}`, title: "Quality check passed", body: `step=${kv.step || "-"}` }); continue; }
      ...
      if (name === "claw_user_reply") {
        for (let i = items.length - 1; i >= 0; i -= 1) {
          if (items[i].type === "tool-card" && items[i].action === "ask_user") { items[i].status = "complete"; items[i].result = `reply=${kv.answer || entry.detail || ""}`; break; }
        }
        continue;
      }
      if (name === "claw_user_interrupt") { items.push({ type: "interrupt", id: `interrupt-${entry.stamp}`, title: "You (mid-run)", body: kv.message || entry.detail || "" }); continue; }
      if (name === "claw_chapter_checkpoint") { items.push({ type: "banner", id: `checkpoint-${entry.stamp}`, title: `Chapter ${kv.chapter || "?"} finished`, body: `planned_total=${kv.planned_total || "-"} | chars=${kv.chars || "-"}` }); continue; }
      if (name === "claw_user_chapter_instruction") { items.push({ type: "banner", id: `chapter-note-${entry.stamp}`, title: `Instruction recorded for chapter ${kv.chapter || "?"}`, body: kv.instruction || entry.detail || "" }); continue; }
      if (name === "claw_user_memory_update") { items.push({ type: "banner", id: `memory-${entry.stamp}`, title: `Memory updated after chapter ${kv.chapter || "?"}`, body: kv.banks || entry.detail || "" }); continue; }
      if (name === "claw_finalize_ready" || name === "claw_loop_complete") { items.push({ type: "banner", id: `complete-${entry.stamp}`, title: "Claw loop complete", body: kv.reason || entry.detail || "" }); continue; }
  ...
  function renderToolCard(item, isLive) {
    ...
    const badgeLabel = item.status === "running" ? "Running" : item.status === "complete" ? "Done" : "Waiting";
    const spinnerHtml = item.status === "running" && isLive ? `<div class="tc-spinner-line"><span class="tc-spinner"></span><span>Executing ${esc(meta.label)}</span></div>` : "";
    const bodyHtml = `${item.params ? `<div class="tc-section-label">Input</div><pre class="tc-code-block">${esc(item.params)}</pre>` : ""}${item.result ? `<div class="tc-section-label">Result</div><pre class="tc-code-block tc-code-block-result">${esc(item.result)}</pre>` : ""}`;
    return `<article class="tool-call-card ${toneClass}" data-tc-id="${esc(item.id)}"><button class="tool-call-header" type="button" data-tc-toggle="${esc(item.id)}"><span class="tool-call-icon">${esc(meta.icon)}</span><span class="tool-call-name">${esc(meta.label)}</span><span class="tool-call-meta"><span class="tool-call-step">step ${esc(String(item.step))}</span><span class="tc-badge ${badgeClass}">${esc(badgeLabel)}</span><span class="tc-chevron">&rsaquo;</span></span></button>${spinnerHtml}<div class="tool-call-body">${bodyHtml || '<div class="tc-empty-note">No structured details yet.</div>'}</div></article>`;
  }
  ...
  function renderProgressFallback(snapshot, status, errorMessage) {
    ...
    return `<div class="claw-progress-fallback${stalled ? " is-stalled" : ""}"><div class="claw-progress-fallback-head"><strong>${esc(stalled ? "Claw may be stalled" : "Claw is still working")}</strong><span class="mini-badge">${esc(phase)}</span></div><div class="claw-progress-fallback-grid"><div class="claw-progress-metric"><span>Status</span><strong>${esc(normalizedStatus || "running")}</strong></div><div class="claw-progress-metric"><span>Chapter</span><strong>${chapter > 0 ? `${chapter}${total > 0 ? ` / ${total}` : ""}` : "-"}</strong></div><div class="claw-progress-metric"><span>Elapsed</span><strong>${esc(`${elapsed}s`)}</strong></div><div class="claw-progress-metric"><span>Words</span><strong>${esc(words ? String(words) : "-")}</strong></div></div><div class="claw-progress-track"><div class="claw-progress-track-fill" style="width:${Math.max(4, Math.min(100, percent || 4))}%"></div></div><div class="claw-progress-fallback-note">${esc(note || "The worker is still updating status, but no structured trace items have been emitted yet.")}</div>${idle >= 0 ? `<div class="claw-progress-fallback-meta">Last log idle: ${esc(String(idle))}s</div>` : ""}${reason ? `<div class="claw-exec-error"><strong>${stalled ? "Attention needed" : "Runtime note"}</strong><div class="claw-exec-error-body">${esc(reason)}</div></div>` : ""}</div>`;
  }

  function renderTimeline(items, isLive, status, errorMessage, snapshot) {
    const normalizedStatus = String(status || "").toLowerCase();
    const errorText = String(errorMessage || "").trim();
    if (!items.length && normalizedStatus === "failed") return renderAssistantRow("Claw failed", `<div class="claw-exec-error"><strong>Run terminated before structured events arrived.</strong><div class="claw-exec-error-body">${esc(errorText || "Unknown error")}</div></div>`, "claw-timeline-body");
    if (!items.length) return renderAssistantRow("Claw execution", renderProgressFallback(snapshot, status, errorMessage), "claw-timeline-body");
    ...
    let title = "Claw execution";
    if (normalizedStatus === "failed") title = "Claw failed";
    else if (askCard) title = "Claw is waiting for your answer";
    else if (hasComplete) title = "Claw completed";
    else if (runningCard && isLive) title = `Claw is working on ${runningCard.action}`;

    const failureHtml = normalizedStatus === "failed" ? `<div class="claw-exec-error"><strong>Run terminated unexpectedly.</strong><div class="claw-exec-error-body">${esc(errorText || "Unknown error")}</div></div>` : "";
  ...
  function renderQuestion(question, sending) {
    return renderAssistantRow("Claw is asking", `<div class="agent-question-card"><div class="agent-question-text">${esc(question)}</div><div class="agent-question-input-row"><textarea class="agent-question-textarea" rows="3" placeholder="Type your answer here"></textarea><button class="agent-question-submit primary-btn" type="button"${sending ? " disabled" : ""}>Reply</button></div><div class="agent-question-status"${sending ? "" : " hidden"}>Sending reply...</div></div>`);
  }
  ...
  function renderCheckpoint(payload, sending, memoryDrafts) {
    ...
    const draftHtml = (memoryDrafts || []).length ? `<div class="chapter-checkpoint-memory-list">${memoryDrafts.map((item, index) => `<div class="checkpoint-memory-pill"><strong>${esc(item.label || item.bank)}</strong><span>${esc(item.content || "")}</span><button type="button" class="checkpoint-memory-remove" data-remove-memory="${index}">x</button></div>`).join("")}</div>` : '<div class="row-meta">No memory additions queued yet.</div>';
    return renderAssistantRow(`Chapter ${esc(String(chapterNo))} complete`, `<div class="chapter-checkpoint-card"><div class="chapter-checkpoint-header"><div><div class="chapter-checkpoint-title">Chapter checkpoint</div><div class="chapter-checkpoint-subtitle">Chapter ${esc(String(chapterNo))} / ${esc(String(plannedTotal))}. Continue, add notes, or update memory before the next chapter.</div></div></div><div class="chapter-checkpoint-chips">${CHECKPOINT_CHIPS.map((chip) => `<button type="button" class="checkpoint-chip" data-chip="${esc(chip)}">${esc(chip)}</button>`).join("")}</div><div class="chapter-checkpoint-input-row"><textarea class="chapter-checkpoint-textarea" rows="3" placeholder="Optional instruction for the next chapter"></textarea><button class="primary-btn chapter-checkpoint-submit" type="button"${sending ? " disabled" : ""}>Continue</button></div><div class="chapter-checkpoint-memory-box"><div class="row-meta checkpoint-memory-heading">Add dynamic memory before continuing</div><div class="chapter-checkpoint-memory-row"><select class="chapter-memory-bank">${CHECKPOINT_MEMORY_BANKS.map((item) => `<option value="${esc(item.slug)}">${esc(item.label)}</option>`).join("")}</select><textarea class="chapter-memory-textarea" rows="2" placeholder="Add a memory note for future steps"></textarea><button class="mini-btn primary chapter-memory-add" type="button"${sending ? " disabled" : ""}>Add memory</button></div>${draftHtml}</div><div class="chapter-checkpoint-status"${sending ? "" : " hidden"}>Sending to Claw...</div><button type="button" class="chapter-checkpoint-skip" data-skip>Continue without notes</button></div>`);
  }

  function renderInterruptBar(isVisible, sending, statusText) {
    if (!isVisible) return "";
    return `<div class="claw-interrupt-bar"><div class="claw-interrupt-copy"><strong>Interrupt Claw mid-run</strong><span>Your message will be injected into the next decision step without leaving this chat page.</span></div><div class="claw-interrupt-input-row"><textarea class="claw-interrupt-textarea" rows="2" placeholder="Tell Claw to change direction, stop after this chapter, tighten the brief, or focus on a different priority"></textarea><button class="primary-btn claw-interrupt-submit" type="button"${sending ? " disabled" : ""}>Send</button></div><div class="claw-interrupt-status"${statusText ? "" : " hidden"}>${esc(statusText || "")}</div></div>`;
  }
  ...
  function init(root) {
    ...
    async function refreshStatus() {
      const response = await fetch(`/api/idea-copilot/${sessionId}/live`, { cache: "no-store" });
    ...
    async function pollQuestion() {
      ...
      try {
        const response = await fetch(`/api/runs/${currentRunId}/pending-question`, { cache: "no-store" });
        const data = await response.json();
    ...
    async function pollCheckpoint() {
      ...
      try {
        const response = await fetch(`/api/runs/${currentRunId}/chapter-complete`, { cache: "no-store" });
        const data = await response.json();
    ...
    async function sendQuestionReply() {
      ...
      renderAll();
      try {
        const response = await fetch(`/api/runs/${currentRunId}/answer`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ answer }) });
    ...
    async function sendCheckpointMessage(message) {
      ...
      renderAll();
      try {
        const response = await fetch(`/api/runs/${currentRunId}/chapter-message`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: message || "", memory_updates: checkpointMemoryDrafts.map((item) => ({ bank: item.bank, content: item.content, source: "checkpoint_ui", kind: "between_chapter_note" })) }) });
    ...
    async function sendInterruptMessage() {
      ...
      renderAll();
      try {
        const response = await fetch(`/api/runs/${currentRunId}/interrupt`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message }) });
