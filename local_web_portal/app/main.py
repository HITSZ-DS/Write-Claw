...
_ZH_FALLBACKS: Dict[str, str] = {
    ...
    "Setup": "设置",
    "Environment": "环境",
    "WriteClaw running": "WriteClaw 运行中",
    "mode": "模式",
    "max_steps": "最大步数",
    "This is the real-time WriteClaw workspace. Start, continue, delete, and review conversations without being kicked out to another page.": "这里是 WriteClaw 的实时写作工作台。新建会话、继续对话、删除会话和查看当前打磨结果，都可以在这一页完成。",
    ...
    "You": "你",
    "Thinking": "正在思考",
    "WriteClaw analysis": "WriteClaw 分析",
    "Next questions": "下一步问题",
    "Current writing brief": "当前写作简报",
    "Suggested next answers:": "建议下一步回答：",
    "Readiness": "准备度",
    "No conversation yet.": "还没有对话记录。",
    "Welcome to WriteClaw": "欢迎来到 WriteClaw",
    "Create a writing session on the left to see the live collaboration here.": "先在左侧创建一个写作会话，这里会实时显示协作内容。",
    "Execution mode": "执行模式",
    "Active sessions": "活跃会话",
    "Running jobs": "运行中任务",
    "Dynamic memory": "动态记忆",
    "WriteClaw Status": "WriteClaw 状态",
    "Start a new writing session": "开始新的写作会话",
    "Describe the premise, genre, cast, tone, constraints, target length, and what you want WriteClaw to help with.": "描述故事设定、题材、角色、语气、约束、目标篇幅，以及你希望 WriteClaw 如何协助你。",
    "Start WriteClaw": "启动 WriteClaw",
    ...
    "Rounds": "轮次",
    "Provider": "提供商",
    "This is not a fake shell around an old flow. The goal is to turn your pipeline into a real writing claw with dynamic memory, capability routing, revision loops, and reusable writing assets.": "这不是套在旧流程外面的假壳子。目标是把你的写作管线改造成真正的 WriteClaw，具备动态记忆、能力调度、修订循环和可复用写作资产。",
    ...
    "Review current and past runs, then open the live WriteClaw execution detail.": "查看当前和历史任务，并进入实时的 WriteClaw 执行详情。",
    "created": "创建于",
    "No runs yet.": "还没有任务。",
    ...
    "Running Jobs": "运行中任务",
    "Total Jobs": "任务总数",
    "Critical environment values that affect WriteClaw runtime behavior.": "影响 WriteClaw 运行时行为的关键环境变量。",
    ...
    "WriteClaw already wires together internal agents, dynamic memory, provider settings, and the executor. MCP is the next layer for external tools.": "WriteClaw 当前已经串起内部 agents、动态记忆、provider 配置和执行器。MCP 会是下一层外部工具接入面。",
    "Writing Skills": "写作技能",
    ...
    "Disable": "禁用",
    "Enable": "启用",
    "See how your existing agents are used by the WriteClaw loop instead of sitting behind a fixed workflow.": "查看现有 agents 如何被 WriteClaw 循环真实调用，而不是躲在固定流程后面。",
    "Loop Config": "循环配置",
    "Execution Mode": "执行模式",
    "Latest Run": "最新运行",
    "Routing Logic": "路由逻辑",
    "WriteClaw observes the current candidate, dynamic memory, issue types, and support outputs before choosing the next agent call.": "WriteClaw 会观察当前候选稿、动态记忆、问题类型和辅助输出，再决定下一次 agent 调用。",
...
def _console_texts(lang: str) -> Dict[str, str]:
    return {
        ...
        "runtime_running": _ui_text(lang, "WriteClaw 运行中", "WriteClaw running"),
        ...
        "models_page_desc": _ui_text(lang, "配置 WriteClaw 使用的提供商、模型与 API Key。", "Configure the providers, models, and API keys used by WriteClaw."),
...
def _memory_index_candidates(run_id: str) -> List[Path]:
    ...
    return [
        (base_root / "vector_db" / "memory" / f"run_{run_id}" / "memory_index.json").resolve(),
        (base_root / "vector_db" / "memory_index.json").resolve(),
    ]


def _load_memory_index_for_run(run_id: str) -> Dict:
    candidates = _memory_index_candidates(run_id) if run_id else [
        (BASE_DIR.parent / "vector_db" / "memory_index.json").resolve(),
    ]
...
def _remember_session_turn(
    ...
    *,
    ...
    try:
        ...
        topic = str(final_topic or state.get("refined_idea") or session.refined_idea or session.original_idea or f"session:{session.id}").strip()
        if not topic:
...
def _build_agent_catalog() -> List[Dict[str, str]]:
    return [
        ...
        {"slug": "writer", "name": "Writer Agent", "role": "draft generation", "desc": "Drafts, continues, and rewrites prose. This is the core writing engine inside WriteClaw."},
...
def _memory_index_path_for_run(run_id: str) -> str:
    candidates = _memory_index_candidates(run_id) if run_id else [
        (BASE_DIR.parent / "vector_db" / "memory" / "memory_index.json").resolve(),
        (BASE_DIR.parent / "vector_db" / "memory_index.json").resolve(),
    ]
    for path in candidates:
        if path.exists():
...
def _console_context(request: Request, db: Session, user: User, active_nav: str) -> Dict:
    ...
    if active_session is None:
        active_session = next((item for item in idea_sessions if item.status not in {"canceled", "deleted"}), None)
    ...
    memory_overview = _build_memory_overview(run_id)
    ...
    story_assets = _build_story_assets(memory_overview, chat_session, session_job)
    ...
    status_summary = {
        ...
        "session_status": chat_session.status if chat_session else ui_text("新建中", "New"),
...
def _start_generation_from_idea_session(db: Session, session: IdeaCopilotSession, user_id: int) -> GenerationJob:
    ...
    job = _create_generation_job(db, user_id, session.provider, final_idea)
...
def _job_detail_texts(language: str) -> Dict[str, str]:
    return {
        "page_title": _lang_text(language, "WriteClaw 运行页", "WriteClaw Run"),
        "page_subtitle": _lang_text(language, "这里展示 WriteClaw 的实时执行状态、Claw 动作轨迹、章节产出和最终结果。", "This page shows WriteClaw's live execution state, Claw action trace, chapter outputs, and final result."),
        "job_title": _lang_text(language, "WriteClaw 运行", "WriteClaw Run"),
        ...
        "back_dashboard": _lang_text(language, "返回 WriteClaw 工作台", "Back to WriteClaw workspace"),
        ...
        "trace_openclaw_detail": _lang_text(language, "WriteClaw 已切换到原生 Claw 循环，不再先走固定 workflow 种子。", "WriteClaw has switched to the native Claw loop instead of seeding a fixed workflow first."),
...
def idea_copilot_confirm(session_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return _redirect_with_notice(
        ...
        message=_ui_text(_ui_language(request), "WriteClaw 已进入创作执行模式。", "WriteClaw has entered creation mode."),
    )
